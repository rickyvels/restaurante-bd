...
import { Download, Play, TrendingUp, Package, Coins, Clock, Sparkles } from 'lucide-react'
...
export default function Reporte() {
  ...
  async function runReporte() {
    ...
    try {
      ...
      setData(rows); setLastRun(fecha); setHistorial(nuevoHistorial); setRan(true)
  ...
  return (
    <div className="relative">
      ...
      <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-orange-500/10 text-orange-500 border border-orange-500/20 mb-2">
        ...
        <div className="flex gap-2">
          <button onClick={runReporte} disabled={loading}
            className="group flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:opacity-50 text-white rounded-xl text-sm font-semibold shadow-lg shadow-orange-500/25 transition-all hover:scale-[1.02]">
      ...
      {ran && data.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="relative overflow-hidden bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
            ...
            <div className="text-xs text-gray-400 mt-1">con más de 2 unidades vendidas</div>
          </div>
          <div className="relative overflow-hidden bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
            ...
            <div className="text-xs text-gray-400 mt-1">{totalVendido} unidades vendidas</div>
          </div>
      ...
      {ran && data.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden mb-6">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">por unidades vendidas</span>
          </div>
      ...
      {/* Historial de reportes ejecutados */}
      {historial.length > 0 && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
            <Clock size={14} className="text-orange-500" /> Historial de reportes
          </h3>
          <div className="space-y-2">
            {historial.map((h, i) => (
              <div key={i} className="flex items-center justify-between text-sm py-2 px-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                <span className="text-gray-500 dark:text-gray-400">{h.fecha}</span>
                <span className="text-gray-700 dark:text-gray-300">
                  {h.productos} productos · <span className="font-semibold text-emerald-600 dark:text-emerald-400">S/ {h.ingresos.toFixed(2)}</span>
                </span>
              </div>
            ))}
          </div>
